package com.duwei.androidplugindemo;
...
public class MainActivity extends BaseActivity {
    ...
    private void click(){
        ...
        String filesDir = this.getCacheDir().getAbsolutePath();     ///data/data//cache目录

        String libPath ="/data/app/com.duwei.pulgin1-2/base.apk";
       // String libPath = filesDir + File.separator +"pulgin1";
        Log.i("inject", "fileexist:"+new File(libPath).exists());
        Log.i("inject", "filesDir:"+filesDir);
       // String libPath = "/data/local/tmp/com.duwei.pulgin1";
        loadResources(libPath);
        //待加载的dex文件路径,解压后的dex存放位置,指向包含本地库(so)的文件夹路径,父级类加载器
        DexClassLoader loader = new DexClassLoader(libPath, filesDir,null, getClassLoader());

        Class<?> clazz = null;
        try {
            clazz = loader.loadClass("com.duwei.pulgin1.MainActivity");

            Class rClazz = loader.loadClass("com.duwei.pulgin1.R$layout");
    ...
    private void loadApkClassLoader(DexClassLoader dLoader){
        try{
            ...
            String libPath = filesDir + File.separator +"Pulgin1.apk";
